...
export default function Food() {
  ...
  const [allergetics, setAllergetics] = React.useState<
  ...
  const handleOptionChange = (e: SelectChangeEvent) => {
    ...
    setTotalCal(
      bmr *
        params1[parseInt(e.target.value) - 1] *
        params2[parseInt(exercise) - 1]
    );
  };
  ...
  const handleExerciseChange = (e: SelectChangeEvent) => {
    ...
    setTotalCal(
      bmr *
        params1[parseInt(option) - 1] *
        params2[parseInt(e.target.value) - 1]
    );
  };

  const handleBreakfastPercentChange = (
    e: Event,
  ...
  const { width, height1 } = useWindowSize();
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box>
          ...
          <Card>
            ...
            <CardContent sx={{ display: "flex", columnGap: 5 }}>
              <Box>
                ...
                <Typography
                  sx={{ color: "#40916C", fontSize: 34, fontWeight: "medium" }}
                >
                ...
                <Box>
                  ...
                  <Slider
                    ...
                    onChange={handleBreakfastPercentChange}
                  ...
                  <Slider
                    value={lunchPercent}
                    onChange={handleLunchPercentChange}
                    valueLabelDisplay="auto"
                    valueLabelFormat={valueLabelFormat}
                    min={0}
                    max={100}
            ...
            <Collapse in={expanded} timeout="auto" unmountOnExit>
              <CardContent sx={{ pt: 0 }}>
                ...
                <Typography variant="body2">
                  BMR is the number of calories a person burns by simply
                  existing. BMR varies based on age, sex, size and genetics. To
                  calculate BMR, a person uses inches for height, pounds for
                  weight, and years for age in the following formulas:
                </Typography>
                <Typography variant="body2">
                  - For men: 66 + (6.2 * weight) + (12.7 * height) - (6.76 *
                  age)
                </Typography>
                <Typography variant="body2">
                  - For women: 655.1 + (4.35 * weight) + (4.7 * height) - (4.7 *
                  age)
                </Typography>

                <Typography variant="h6">Activities Points</Typography>
                <Typography variant="body2">
                  Points for activity levels are: 1.2 points for a person who
                  does little to no exercies; 1.37 points for a acitve person
                  who does light exercise 1-3 days a week; 1.725 points for a
                  very active person who exercise hard 6-7 days a week.
                </Typography>
                <Typography variant="body1">
                  When the BMR is calculated, and the activties points are
                  determined, the two scores are multiplied. The total is the
                  number of calories burned on an average day.
                </Typography>
              </CardContent>
            </Collapse>
          </Card>
        ...
        <div>
          {confetti ? <Confetti width={width} height={height1} /> : <div></div>}
        </div>
